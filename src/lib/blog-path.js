 ...
 * `ui.router()` paths into the URL hash; percent-encoded CJK is passed through to
 * GraphQL as literal folder names and breaks `relativePath`. Next.js accepts IRIs and
 * decodes `%…` in the request URL when present.
 */

/**
 * If a segment is still percent-encoded, decode once. Invalid `%` sequences unchanged.
 * @param {string} seg
 * @returns {string}
 */
